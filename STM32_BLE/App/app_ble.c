 ...
 * Advertising Data
 */
/* Manufacturer-specific blob removed (was 12 bytes total: len+type+ID+8B
 * 'Sebkabob') — iOS finds the device by name + service UUID, the blob
 * wasn't consumed anywhere. Shorter packet → less radio on-air time per
 * adv burst on each of the 3 primary channels. */
uint8_t a_AdvData[] =
{
  2, AD_TYPE_FLAGS, FLAG_BIT_LE_GENERAL_DISCOVERABLE_MODE|FLAG_BIT_BR_EDR_NOT_SUPPORTED,
  9, AD_TYPE_COMPLETE_LOCAL_NAME, 'W', 'a', 't', 'c', 'h', 'D', 'o', 'g',
    ...
    if (m24cxx_init(&bd_eeprom, &hi2c1, EEPROM_I2C_ADDRESS) == M24CXX_Ok) {
      ...
      if (bd_address_override) {
        ...
        if (m24cxx_read(&bd_eeprom, EEPROM_DEVICE_INFO_ADDR + 1, stored_addr, 6) == M24CXX_Ok) {
          ...
          (void)aci_hal_write_config_data(CONFIG_DATA_PUBADDR_OFFSET,
